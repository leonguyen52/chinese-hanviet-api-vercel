...
if (isset($_GET['api_request'])) {
    #Specify API URL here
    $api_base = 'https://cndict.lukobi.com';
  
    $endpoint = $_GET['endpoint']; // 'tughep' or 'tudon'
    $word = $_GET['word'];
    
    $url = $api_base . '/' . $endpoint . '?word_like=' . urlencode($word);
...
function containsChinese(str) {
        ...
        async function searchWord(word, type) {
            try {
                ...
                const response = await fetch(`index.php?api_request=1&endpoint=${endpoint}&word=${encodeURIComponent(word)}`);
                if (!response.ok) {
